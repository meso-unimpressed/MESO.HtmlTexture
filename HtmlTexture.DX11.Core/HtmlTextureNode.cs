...
namespace VVVV.DX11.Nodes
{
    ...
    public abstract class HTMLTextureNode : IDX11ResourceHost, IDisposable
    {
        ...
        [Input("Size", DefaultValues = new double[] {DEFAULT_WIDTH, DEFAULT_HEIGHT})]
        public IDiffSpread<Vector2D> FSize;

        [Input("Parent Handle")]
        public IDiffSpread<int> FParentHandle;
        ...
        public void Update(int SpreadMax)
        {
            ...
            if (createBrowser)
            {
                ...
                windowInfo.SetAsWindowless(new IntPtr(FParentHandle[0]));
            ...
            if (FSize.IsChanged)
            {
                if (FSize.SliceCount > 0 && FSize[0].x > 0 && FSize[0].y > 0)
                {
                    lock (bLock)
                    {
                        width = (int)FSize[0].x;
                        height = (int)FSize[0].y;
        ...
        class EvaluateTask : CfrTask
        {
            ...
            void Task_Execute(CfrEventArgs e)
            {
                ...
                try
                {
                    ...
                    var result = context.Eval(code, "", 0, out var retval, out var ex);
